...
(async () => {
  ...
  await check("new resume is selectable when creating an opportunity", async () => {
    ...
    const tmp = path.join(os.tmpdir(), "jobpilot-v072-resume.pdf");
    ...
    await page.getByRole("button", { name: /开始解析/ }).click();
    const resumeSelectText = await page.locator(".composer-grid select").nth(3).innerText();
